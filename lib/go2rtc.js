    ...
    async init(cameras) {
        ...
        if (cameras) {
            ...
            for (const camera of cameras) {
                ...
                config.streams[`${camera.deviceId}_snapshot`] =
                    `exec:${dirname(fileURLToPath(new URL('.', import.meta.url)))}/scripts/start-stream.sh ${camera.deviceId} snapshot ${camera.deviceTopic} {output}#killsignal=15`
            }
            try {
