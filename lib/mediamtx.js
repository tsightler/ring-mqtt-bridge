...
export default new class MediaMTX {
    ...
    async init(cameras) {
        ...
        if (cameras) {
            ...
            for (const camera of cameras) {
                config.paths[`${camera.deviceId}_live`] = {
                    runOnDemand: `"${dirname(fileURLToPath(new URL('.', import.meta.url)))}/scripts/start-stream.sh" ${camera.deviceId} live ${camera.deviceTopic} rtsp://localhost:8554/${camera.deviceId}_live`
                },
                config.paths[`${camera.deviceId}_event`] = {
                    runOnDemand: `"${dirname(fileURLToPath(new URL('.', import.meta.url)))}/scripts/start-stream.sh" ${camera.deviceId} live ${camera.deviceTopic} rtsp://localhost:8554/${camera.deviceId}_event`
                }
            }
            try {
        ...
        this.mediamtxProcess = spawn('mediamtx', ['/home/tsightler/GitHub/ring-mqtt/config/mediamtx.yaml'], {
