...
export default new class RingMqtt {
    ...
    setupSubscriptions() {
        ...
        this.devices$.pipe(
            ...
            this.updateDeviceStates(devices)
        })
    }
        ...
        devices.forEach(device => {
            if (device.state$) {
                device.state$
                    ...
                    .subscribe(state => {
                        if (this.mqttConnected$.value) {
                            try {
                                const topic = `ring/${device.locationId}/${device.deviceId}/state`
                                const message = JSON.stringify(state)
                                utils.mqtt.publish(topic, message, { retain: true })
                            } catch (error) {
                                debug(`Error publishing state for device ${device.deviceId}:`, error)
                            }
                        }
                    })
            }
